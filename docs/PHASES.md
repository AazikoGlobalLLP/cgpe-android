...
| 98 | **EAS Update (OTA)** — a JS fix no longer needs an APK | ✅ **DONE — device-confirmed** 2026-09-02 (`11eff09`; build 6 = `80df5c5a`, versionCode 6). Owner ran the full round trip on a real handset: install → banner appeared unprompted → tap → restart → `Settings › Version` gained the `· u…` suffix. `expo-updates@~57.0.21`, `runtimeVersion` **fingerprint** policy (chosen after MEASURING it works here — the Windows trap did not reproduce), `updates.url` + `checkAutomatically: ON_LOAD` + `fallbackToCacheTimeout: 0`, `channel` per profile. Pure seams `lib/otaPolicy.ts` (throttle + banner gate) + `lib/ota.ts` (lazy-require, fail-quiet, no `data/health`); `UpdateBanner` OFFERS a restart, never performs one. `formatVersionLine` names the update so OTA does not silently break the build discriminator. Publish: `npx eas-cli update --channel preview --message "…" --environment preview`. Gates: tsc 0 · **1378 / 84 files** · eslint 0 on touched · web-export exit 0. 🔴 **Only build 6+ can ever receive an update; each older handset needs ONE manual install first — the rollout is the owner's.** ⚠️ `eas.json`/`.easignore`/`.gitignore` are fingerprint SOURCES: edit one and an update silently reaches nobody |
