...
| `(tabs)/more.tsx` | The real role-gating surface — builds its groups from `capabilitiesOf()`. Since Phase 10, also drops any row whose module is in `nav.hidden`; group *structure* (titles/order) is still hard-coded and does **not** read `nav.more_sections`. |
...
- **`nav.tabs` and `nav.hidden` are live as of Phase 10** (2026-08-11) — `(tabs)/_layout.tsx`
  ...
  row, group, or quick-action tile whose module is in `nav.hidden`. `nav.more_sections`
  (title/grouping) is still not consumed — a deliberate scope cut, `docs/spec/PHASE-10.md` D-3 —
  and the previously-documented gap (`ADMIN_PANEL_SYNC.md` §9) is otherwise closed.
...
| Navigation | **Fixed in Phase 10** (2026-08-11). `nav.tabs`/`nav.hidden`, stored and served correctly since before this app existed, now actually change the bottom bar and the More screen — the `ADMIN_PANEL_SYNC.md` §9 gap is closed. `nav.more_sections` (title/grouping) is still not consumed — deliberate, `docs/spec/PHASE-10.md` D-3 — and `prospects`/`tickets` still cannot become physical bottom tabs without a route restructuring out of this phase's scope. |
