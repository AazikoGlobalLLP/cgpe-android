...
| 4 | App 63 MB grows to 125 MB after use | ~~Map tile cache~~ **CORRECTED 2026-08-26 (Phase 77): tiles are only PART of it and the smaller part.** Both `LeafletMap` mounts sit behind `canSeeLiveLocation`, so an ordinary advisor never downloads a tile. The **every-user** leak is the picked-file copies: `DocumentSource` passes `copyToCacheDirectory:true`, so every attached document/photo (≤10 MB each) is copied into `<cache>/DocumentPicker` and `<cache>/ImagePicker` and nothing ever deleted them. A third slice — the APK, extracted native libs and ART/dex profiles — is **not a cache and not recoverable**. | `[m]` | ✅ SHIPPED: Settings › Storage › "Clear cached downloads" |
| 5 | Splash layout broken, text invisible | Two splashes, three measured faults (Phase 77). **"Layout broken"** = the mark JUMPED ~50% at the handover — the plugin fits the 827×975 logo into an `imageWidth:190` **square**, so native renders 161×190 dp while the JS splash redrew it at ~242 dp and scaled it in. **"Text invisible"** = the tagline was `c.muted` #738296 at 13 px on white, a measured **3.92:1** (WCAG AA needs 4.5:1) — and in **dark mode** the whole thing was worse: the logo is dark-ink art (0% of its opaque pixels lighter than 0.75 luminance) on `#070c14`, after a hard white→black flash. The native splash having no text at all is by design and cannot change (Android 12 splash API takes an icon only). | `[m]` | ✅ SHIPPED: JS splash now continues the native one; `app.json` deliberately untouched |
| 6 | LIC plans 102/113/122/165/172/180/181/195 show "Unnamed" | **DATA, not a bug — but the mechanism was misread.** `plan_name` is `null` for **11** rows (not 8: also 5, 836, 904) in `data/lic_plans_library.json`. **The app never sees that null:** `productIngestion.js:121` substitutes `String(d.plan_name \|\| 'Unnamed plan')` on ingest and `:146` hands the placeholder back out, so the wire carries the truthy STRING "Unnamed plan" and the screen's own `\|\| 'Unnamed plan'` is dead code. Verified on deployed `origin/main` (990c660). | `[data]+[m]` | ✅ SHIPPED: `adaptLicPlan` keys on the sentinel → "LIC Plan 102". **Owner still owes the 11 real names.** Expect 11 rows to change, not 8 |
| 7 | Deploy to Play Store + App Store today | See §B2 — **Apple is blocked**, Play is possible but not same-day | `[ops]+[m]` | see §B2 |
| 8 | More, then back to Today, goes black/empty | ❌ **THE PRIME SUSPECT IS RULED OUT (Phase 77, 2026-08-26).** `Appear`'s `cancelAnimation` cleanup cannot be it: its effect deps are constants at every Home call site, so the cleanup runs only at UNMOUNT — there is no third occasion on a tab switch. react-freeze is off (`ENABLE_FREEZE = false`; `enableFreeze()` called nowhere), there is no `unmountOnBlur`, `BottomTabView` only appends to `loaded`, and reanimated 4.5 bakes a settled `opacity: 1` into React's committed props within ~1 s. **Still undiagnosed.** | `[m]` | ⚠️ needs a device repro FIRST — two zero-build tests in `docs/PHASES.md` §"Phase 77 leftovers" |
